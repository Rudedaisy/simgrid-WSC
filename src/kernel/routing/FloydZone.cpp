...
XBT_LOG_NEW_DEFAULT_SUBCATEGORY(ker_routing_floyd, ker_routing,
                                "Kernel Floyd Routing");
    ...
    if (link_table_.size() != table_size)
    {
        ...
        for (auto& cost : cost_table_)
            cost.resize(table_size,
                        ULONG_MAX); /* link cost from host to host */
...
void FloydZone::get_local_route(const NetPoint* src, const NetPoint* dst,
                                Route* route, double* lat)
{
    ...
    do
    {
        ...
        if (pred == -1)
            throw std::invalid_argument(
                xbt::string_printf("No route from '%s' to '%s'",
    ...
    while (not route_stack.empty())
    {
        ...
        {
            get_global_route(prev_dst_gw, e_route->gw_src_, route->link_list_,
                             lat);
        }
...
void FloydZone::add_route(NetPoint* src, NetPoint* dst, NetPoint* gw_src,
                          NetPoint* gw_dst,
                          ...
                          bool symmetrical)
{
    ...
    if (gw_dst && gw_src)  // netzone route (to adapt the error message, if any)
        xbt_assert(nullptr == link_table_[src->id()][dst->id()],
                   ...
                   src->get_cname(), gw_src->get_cname(), dst->get_cname(),
                   gw_dst->get_cname());
    else
        xbt_assert(nullptr == link_table_[src->id()][dst->id()],
    ...
    link_table_[src->id()][dst->id()] = std::unique_ptr<Route>(
        new_extended_route(get_hierarchy(), gw_src, gw_dst,
                           get_link_list_impl(link_list, false), true));
    predecessor_table_[src->id()][dst->id()] = src->id();
    cost_table_[src->id()][dst->id()] =
        link_table_[src->id()][dst->id()]->link_list_.size();

    if (symmetrical)
    {
        if (gw_dst &&
            gw_src)  // netzone route (to adapt the error message, if any)
            xbt_assert(nullptr == link_table_[dst->id()][src->id()],
                       ...
                       dst->get_cname(), gw_dst->get_cname(), src->get_cname(),
                       gw_src->get_cname());
        else
            xbt_assert(nullptr == link_table_[dst->id()][src->id()],
        ...
        if (not gw_src || not gw_dst)
            XBT_DEBUG("Load Route from \"%s\" to \"%s\"", dst->get_cname(),
                      src->get_cname());
        else
            XBT_DEBUG("Load NetzoneRoute from \"%s(%s)\" to \"%s(%s)\"",
                      dst->get_cname(), gw_src->get_cname(), src->get_cname(),
                      gw_dst->get_cname());

        link_table_[dst->id()][src->id()] = std::unique_ptr<Route>(
            new_extended_route(get_hierarchy(), gw_src, gw_dst,
                               get_link_list_impl(link_list, true), false));
        predecessor_table_[dst->id()][src->id()] = dst->id();
        cost_table_[dst->id()][src->id()] =
            link_table_[dst->id()][src->id()]
                ->link_list_.size(); /* count of links, old model assume 1 */
    }
}

// aux functions for floyd routing
unsigned long* floyd_warshall_blocked_init(const int n, const int block_size,
                                           int* n_blocked)
{
    int n_oversized;
...
inline void floyd_warshall_in_place(unsigned long* C, const unsigned long* A,
                                    const unsigned long* B, signed long* aux_C,
                                    signed long* aux_B, const int ori_n,
                                    const int b, const int n, const int k_base,
                                    const int i_base, const int j_base,
                                    const int host_id, const int io_router_id)
{
    for (int k = 0; k < b; k++)
    {
        if (k + k_base >= ori_n) break;
        int kth = k * n;
        for (int i = 0; i < b; i++)
        {
            if (i + i_base >= ori_n) break;
            for (int j = 0; j < b; j++)
            {
                if (j + j_base >= ori_n) break;
                if (k + k_base == io_router_id && i + i_base != host_id &&
                    j + j_base != host_id && i + i_base != io_router_id &&
                    j + j_base != io_router_id)
                    continue;
                unsigned long sum = A[i * n + k] + B[kth + j];
                if (A[i * n + k] < ULONG_MAX && B[kth + j] < ULONG_MAX &&
                    (C[i * n + j] > sum || C[i * n + j] == ULONG_MAX))
                {
                    C[i * n + j] = sum;
...
void floyd_warshall_blocked(unsigned long* output, signed long* aux,
                            const int ori_n, const int n, const int b,
                            const int io_router_id, const int host_id)
{
    ...
    for (int k = 0; k < blocks; k++)
    {
        floyd_warshall_in_place(
            &output[k * b * n + k * b], &output[k * b * n + k * b],
            &output[k * b * n + k * b], &aux[k * b * n + k * b],
            &aux[k * b * n + k * b], ori_n, b, n, k * b, k * b, k * b, host_id,
            io_router_id);
#pragma omp parallel for
        for (int j = 0; j < blocks; j++)
        {
            if (j == k) continue;
            floyd_warshall_in_place(
                &output[k * b * n + j * b], &output[k * b * n + k * b],
                &output[k * b * n + j * b], &aux[k * b * n + j * b],
                &aux[k * b * n + j * b], ori_n, b, n, k * b, k * b, j * b,
...
#pragma omp parallel for
        for (int i = 0; i < blocks; i++)
        {
            if (i == k) continue;
            floyd_warshall_in_place(
                &output[i * b * n + k * b], &output[i * b * n + k * b],
                &output[k * b * n + k * b], &aux[i * b * n + k * b],
                &aux[k * b * n + k * b], ori_n, b, n, k * b, i * b, k * b,
                host_id, io_router_id);
            for (int j = 0; j < blocks; j++)
            {
                if (j == k) continue;
                floyd_warshall_in_place(
                    &output[i * b * n + j * b], &output[i * b * n + k * b],
                    &output[k * b * n + j * b], &aux[i * b * n + j * b],
                    &aux[k * b * n + j * b], ori_n, b, n, k * b, i * b, j * b,
                    host_id, io_router_id);
            }
        }
    }
...
{
#ifdef _COMPRESS
    bxz::ifstream ifs("FloydRoutingCheckpoint.txt");
#else
        ...
        for (unsigned int i = 0; i < table_size; i++)
        {
            ...
            if (not route)
            {
                ...
                route->link_list_.push_back(
                    get_network_model()->loopback_.get());
    ...
    int block_size = 256;
    unsigned long* _ct = floyd_warshall_blocked_init(table_size, block_size,
                                                     &blocked_table_size);
    signed long* _pret = (signed long*)floyd_warshall_blocked_init(
        table_size, block_size, &blocked_table_size);

    for (int i = 0; i < table_size; i++)
    {
        memcpy(_ct + i * blocked_table_size, cost_table_[i].data(),
               sizeof(unsigned long) * table_size);
        memcpy(_pret + i * blocked_table_size, predecessor_table_[i].data(),
               sizeof(unsigned long) * table_size);
    }

    floyd_warshall_blocked(_ct, _pret, table_size, blocked_table_size,
                           block_size, io_router_id, host_id);

    for (int i = 0; i < table_size; i++)
    {
        memcpy(cost_table_[i].data(), _ct + i * blocked_table_size,
               sizeof(unsigned long) * table_size);
        memcpy(predecessor_table_[i].data(), _pret + i * blocked_table_size,
               sizeof(unsigned long) * table_size);
    }
    ...
    //         {
    //             // Prevent pe-to-pe comm from taking the path through
    //             io_router if (c == io_router_id && a != host_id && b
    //             != host_id &&
    //                 a != io_router_id && b != io_router_id)
    //                 continue;
    //             if (cost_table_[a][c] < ULONG_MAX &&
    //                 cost_table_[c][b] < ULONG_MAX &&
    //                 (cost_table_[a][b] == ULONG_MAX ||
    //                  (cost_table_[a][c] + cost_table_[c][b] <
    //                   cost_table_[a][b])))
    //             {
    //                 cost_table_[a][b] = cost_table_[a][c] +
    //                 cost_table_[c][b]; predecessor_table_[a][b] =
    //                 predecessor_table_[c][b];
    //             }
    //         }
    //     }
    // }
