...
XBT_LOG_NEW_DEFAULT_SUBCATEGORY(ker_routing_wsc, ker_routing,
                                "Kernel WSC Routing");
...
void WSCZone::get_local_route(const NetPoint* src, const NetPoint* dst,
...
void WSCZone::add_route(NetPoint* src, NetPoint* dst, NetPoint* gw_src,
                        NetPoint* gw_dst,
                        ...
                        bool symmetrical)
{
    ...
    /* Roundabout way to infer WSC dimensions. Find X dimension by the order of
     * adding routes. CAUTION: unstable to loop order of gen_config */
    if (!dimensions_inferred && src->id() == contiguous_x &&
        dst->id() == contiguous_x + 1)
        contiguous_x++;

    // std::cout << "Linking " << src->get_cname() << "(" << src->id() << ") to
    // " << dst->get_cname() << "(" << dst->id() << ")" << std::endl;
}
...
void WSCZone::do_seal()
{
    ...
    if (!dimensions_inferred)
    {
        ...
        // std::cout << "Inferred wafer dims " << wsc_x << "x" << wsc_y <<
        // std::endl;
    ...
    // unsigned perim_dists[4] = {0}; // used to find which perimeter is closest
    // to dst_pe. Format: [N,S,W,E]
    unsigned src_pe;  // = 1;
    unsigned dst_pe;
        ...
        for (unsigned src_x = 0; src_x < wsc_x; src_x++)
        {
            for (unsigned dst_y = 0; dst_y < wsc_y; dst_y++)
            {
                for (unsigned dst_x = 0; dst_x < wsc_x; dst_x++)
                {
                    ...
                    {
                        if (dst_y > src_y)
                            predecessor_table_[src_pe][dst_pe] = dst_pe - wsc_x;
                        else
                            predecessor_table_[src_pe][dst_pe] = dst_pe + wsc_x;
                    }
                }
    ...
    for (unsigned dst_y = 0; dst_y < wsc_y; dst_y++) {
      for (unsigned dst_x = 0; dst_x < wsc_x; dst_x++) {
        ...
        if (perim_dists[0]<=perim_dists[1] && perim_dists[0]<=perim_dists[2] &&
    perim_dists[0]<=perim_dists[3]) {
          // N
        ...
        } else if (perim_dists[1]<=perim_dists[0] &&
    perim_dists[1]<=perim_dists[2] && perim_dists[1]<=perim_dists[3]) {
          // S
          ...
          predecessor_table_[io_router_id][dst_pe] = temp1;
          predecessor_table_[dst_pe][io_router_id] = temp2;
        } else if (perim_dists[2]<=perim_dists[0] &&
    perim_dists[2]<=perim_dists[1] && perim_dists[2]<=perim_dists[3]) {
          // W
